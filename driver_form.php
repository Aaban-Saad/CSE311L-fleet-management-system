    ...
    if (empty($name) || empty($contact) || empty($licence) || empty($status)) {
        ...
        if ($is_edit) {
            ...
            try {
                // Debug information
                echo "<pre>";
                echo "Attempting to insert driver with the following data:\n";
                echo "Name: $name\n";
                echo "Contact: $contact\n";
                echo "License: $licence\n";
                echo "Status: $status\n";
                echo "Vehicle ID: " . ($vehicle_id ? $vehicle_id : "NULL") . "\n";
                echo "Hire Date: " . ($hire_date ? $hire_date : "NULL") . "\n";
                echo "Salary: " . ($salary ? $salary : "NULL") . "\n";
                echo "</pre>";
                
                $stmt = $user_conn->prepare("INSERT INTO Drivers (name, contact, licence, status, vehicle_id, hire_date, salary) VALUES (?, ?, ?, ?, ?, ?, ?)");
                if (!$stmt) {
                    throw new Exception("Prepare failed: " . $user_conn->error);
                }
                
                $bind_result = $stmt->bind_param("ssssids", $name, $contact, $licence, $status, $vehicle_id, $hire_date, $salary);
                if (!$bind_result) {
        ...
        <div class="bg-white/10 rounded-lg p-6 backdrop-blur-sm max-w-2xl mx-auto">
            ...
            <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"] . ($is_edit ? "?id=" . $driver['driver_id'] : "")); ?>" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="date" id="hire_date" name="hire_date" value="<?php echo $driver['hire_date']; ?>" class="w-full p-2 rounded bg-black/30 text-white">
                </div>
